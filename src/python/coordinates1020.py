...
def get_1020coords():
    ...
    for line in text_source.split('\n'):
        ...
        coords[label] = tuple(float(ix) for ix in xyz)

    return coords

def main():
    mensia_electrodes = 'O1 O2 Oz Cz C3 C4 Fz F8 F7 Fpz'.split()
    ...
    for electrode in mensia_electrodes:
        x,y,z = coords[electrode]
        X.append(x)
        Y.append(y)
        Z.append(z)
    ...
    xhead = np.sin(circle)
    yhead = np.cos(circle)
    ax.plot(xhead,yhead,'k')

    ax.plot(Y,X,'bo') # swap X and Y since 10-20 system is defined like that
